...
def preprocess_subject(sub):
    ...
    # # Read the Image list
    # Sub_Images_list = pd.read_csv(os.path.join(SubMatrix_path, "Sub_matrix", sub + "_randomized_matrix_702.csv"), header=None)
    ...
    # # Read raw data
    # raw = mne.io.read_raw_bdf(os.path.join(file_path, sub, sub + ".bdf"))
    ...
    # stimuli_epochs_plot_before_NoRestriction.savefig(os.path.join(SubMatrix_path,sub + "_stimuliEEG_epochs_before(All events).png"))
    # stimuli_epochs_plot_before_NoRestriction = stimuli_epochs_NoRestriction.average('eog').plot(**plot_kwargs)
    # stimuli_epochs_plot_before_NoRestriction.set_size_inches(27, 16)
    # stimuli_epochs_plot_before_NoRestriction.savefig(os.path.join(SubMatrix_path,sub + "_stimuliEOG_epochs_before(All events).png"))

    #EEG & EOG plots (Use  AutoReject criteria)
    AutoReject = get_rejection_threshold(epochs_NoRestriction)
    epochs_autoreject = mne.Epochs(raw, events, tmin=-0.1, tmax=0.4, reject = AutoReject, picks = EEG_64channels+eog_channels)
    ...
    # stimuli_epochs_plot_before_AutoReject.savefig(os.path.join(SubMatrix_path,sub + "_stimuliEEG_epochs_before(AutoReject).png"))
    # stimuli_epochs_plot_before_AutoReject = stimuli_epochs_AutoReject.average('eog').plot(**plot_kwargs)
    # stimuli_epochs_plot_before_AutoReject.set_size_inches(27, 16)
    # stimuli_epochs_plot_before_AutoReject.savefig(os.path.join(SubMatrix_path,sub + "_stimuliEOG_epochs_before(AutoReject).png"))

    #EEG & EOG plots (Use self-defined reject criteria)
    epochs = mne.Epochs(raw, events, tmin=-0.1, tmax=0.4, reject = reject_criteria, flat = flat_criteria, picks = EEG_64channels+eog_channels)
    epochs.load_data()
    ...
    # stimuli_epochs_plot_before.savefig(os.path.join(SubMatrix_path,sub + "_stimuli_epochsEEG_before(with reject criteria).png"))
    # stimuli_epochs_plot_before = stimuli_epochs.average('eog').plot(**plot_kwargs)
    # stimuli_epochs_plot_before.set_size_inches(27, 16)
    # stimuli_epochs_plot_before.savefig(os.path.join(SubMatrix_path,sub + "_stimuli_epochsEOG_before(with reject criteria).png"))
    ####################################################################################################################
    ...
    # stimuli_epochs_clean_plain.save(os.path.join(SubMatrix_path, sub + "-OC-epo.fif"), overwrite = False)
    ...
    # stimuli_epochs_eogout_plot.savefig(os.path.join(SubMatrix_path,sub + "_stimuli_epochs_after_OC(with reject criteria).png"))
    ...
    # stimuli_epochs_clean_plain_AutoReject.save(os.path.join(SubMatrix_path, sub + "-OC-AutoReject-epo.fif"), overwrite = False)
    ...
    # stimuli_epochs_eogout_plot_AutoReject.savefig(os.path.join(SubMatrix_path,sub + "_stimuli_epochs_after_OC(AutoReject).png"))
    ...
    # stimuli_epochs_clean_plain_NoRestriction.save(os.path.join(SubMatrix_path, sub + "-OC-NoRestriction-epo.fif"), overwrite = False)
    ...
    # stimuli_epochs_eogout_plot_NoRestriction.savefig(os.path.join(SubMatrix_path,sub + "_stimuli_epochs_after_OC(All events).png"))
    ####################################################################################################################
    ...
    stimuli_epochs_csd.save(os.path.join(SubMatrix_path,sub + "-OC&CSD-epo.fif"), overwrite = False)
    # evoked_plot = stimuli_epochs_csd.average().plot(**plot_kwargs)
    # evoked_plot.set_size_inches(27, 16)
    # evoked_plot.savefig(os.path.join(SubMatrix_path,sub + "_stimuli_epochs_after_OC&CSD(with reject criteria).png"))
    ...
    stimuli_epochs_csd_AutoReject.save(os.path.join(SubMatrix_path,sub + "-OC&CSD-AutoReject-epo.fif"), overwrite = False)
    ...
    # evoked_plot_AutoReject.savefig(os.path.join(SubMatrix_path,sub + "_stimuli_epochs_after_OC&CSD(AutoReject).png"))
    ...
    stimuli_epochs_csd_NoRestriction.save(os.path.join(SubMatrix_path,sub + "-OC&CSD-NoRestriction-epo.fif"), overwrite = False)
    ...
    # evoked_plot_NoRestriction.savefig(os.path.join(SubMatrix_path,sub + "_stimuli_epochs_after_OC&CSD(All events).png"))
    ####################################################################################################################
