...
def convert_from_cv2_to_image(img: np.ndarray) -> Image.Image:
    ...
    return Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB)) # type: ignore

def convert_from_image_to_cv2(img: Image.Image) -> np.ndarray:
    # return np.asarray(img)
    return cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR) # type: ignore

class ToRetinalGanglionCellSampling(object):
    def __init__(self, fov=20, out_size=256, decomp=0, type=1, series=0, image_shape:tuple=None, n_processes=-1, dtype=np.uint8): # type: ignore
        super().__init__()
